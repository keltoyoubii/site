...
(function () {
  ...
  projects.forEach((p, i) => {
    ...
    if (p.duo && isVideo && p.bunny && p.bunny.length >= 2) {
      ...
      mediaHtml = `<div class="work__media work__vid is-video${bunnyId ? " is-bunny" : vimeoId ? " is-vimeo" : ""}" ${attr}>${inner}</div>`;
    } else {
      ...
      entries.forEach((e) => {
        ...
        if (e.isIntersecting) {
          if (!host.querySelector("iframe")) {
            ...
            if (bunny) {
              f.src = `https://player.mediadelivery.net/embed/${bunny}?autoplay=true&loop=true&muted=true&controls=false&preload=true&responsive=true`;
